...
```
charts/
├─ src/core.js         纯计算（比例尺/刻度/堆叠/分箱/四分位/缺口检测/降采样），不碰 DOM
├─ src/line.js         bar.js          渲染层，又薄又笨
├─ src/theme.css       主题层：组件不认识颜色，只引用 token
├─ tests/core.test.js  42 个单元测试（node --test，零依赖，约 80ms）
...
**已知局限**：只有折线和柱状；渲染层没有自动化测试；没有类型与发布流程。

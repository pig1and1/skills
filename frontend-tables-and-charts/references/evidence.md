...
| **规则立了，但没人拿它量范本 —— 这次是字重**：§11 写着"最多两个字重"，而六个参考实现实测用了**七种**（400 / 500 / 550 / 560 / 600 / 620 / 640），**20 处违规**。更糟的是**阶梯表自己给表头配的是 400**（和正文同一个字重），于是 §2 那条"**靠字重 + 字色区分表头**"**根本不成立** | **和第 20 轮那次字号散值同一类，而且我当时只做了一半**：字号量了、归位了；**字重压根没量**。相隔十几轮，是**第 6 次独立验证的检查器**（它量自己的页面时读出 `{400, 550, 620}` 三个值）把它捅出来的 —— **不是我的任何检查**。已全部归位到 400 / 620，表头那一档从 400 改成 620，并给检查器补了第 9b 项（**已证伪**）。⚠️ **"立了规则"和"用规则查过所有范本"是两件事** —— 漏掉的那一半，只能等别人踩到。 |
| **§9 的表格把"间隔 > 3 倍中位值即断开"写成无条件规则**，而**同一节 30 行后**说这条拿它去套每日下单量、会把"连续 0 单"误判成缺测 —— **规则在表格里、纠正在正文里，而读者是从表格取用的** | 第 6 次验证指出。**"我后来补了一段说明"不等于"原来那条规则被修正了"** —— 两者在文档里的**取用优先级**完全不同：表格是"照着做"，正文是"背景"。已在那**一行表格里**加上限定（只对「稀疏上报」成立）并指向后面的小节。 |
| **§13 那段"§9 说…"是引用旧报告的原话，却没标明是旧的** —— 第 6 次验证的构建者把它读成现行声称，在报告里写成"§13 声称 §9 说了某事" | **引用旧结论时，必须让人一眼看出它是旧的。** 已把那段降为嵌套引文并加前置说明。⚠️ **同一类问题在同一个工程里出现了两次**：上次是 `evidence.md` 的"踩过的坑"一节被读成现状，这次是 §13 的引文 —— **两次都是"历史记录没说自己过时"。** |
| **技能两次说"先问用户"、一次说"必须问出来，不能猜"，却没说"无人可问时怎么办"** —— 出路只藏在一个表格单元格和一条来源注里 | 第 6 次验证指出：**"无人可问"是常态，不是例外**（隔离会话，或对方就是"你自己看着办"）。已立成规则：**不许猜，把口径写进界面**（图注 / 表头 / 状态区），判据是"**读者能不能从界面上知道你选了哪个口径**"。**默认值也是一个决定，而决定必须让人看得见。** |
| **§12 的清单里没有"先确认这条断言真的能失败"** —— 第 6 次构建的一条"横向滚动后首列不动"断言，在 1280px 下**恒真**（表格总宽 840px，压根没超出容器），是变异测试把它照出来的 | 已补为清单第 1 条。**"测了但没测到触发条件"和"没测"一样危险** —— 而这正是 `evidence.md` 里反复出现的形状，**却一直没进清单**。 |
| §12 那四条 grep 是 **POSIX 写法**，而这个 skill 的参考实现与检查都在 Windows 上跑 | 已注明 PowerShell 的等价替换（`Select-String` / `Where-Object -notmatch`）。**照抄一条在当前机器上跑不起来的命令，等于没检查。** |
| **第 6 次验证担心 `query-console.html` 用 ES module 会影响"双击即用"，而实测是：它没事** —— 内联 `<script type="module">` **不抓取任何东西**，`file://` 下正常运行（点提交有反应）。我把它测准成四格：内联无 import ✅ · 内联 + `import` ❌ · `type=module src=` ❌ · 普通 `script src=` ✅ | **"它用了 module"不是问题，"它要抓取"才是。** 担心本身是对的，结论是错的 —— 而**只有实测能分开这两件事**。已把这道边界写进 §15（和 `pushState` 那条并列，都是 `file://` 的平台行为）。⚠️ **这次实测还暴露了我的冒烟测试的一个盲区**：模块/资源被挡**不抛未捕获异常**，只出现在 **Log 域**，而它只监听 Runtime —— **整类失败都看不见**。 |
| 给冒烟测试补上 `Log` 监听后，**第一版立刻"抓到"一个缺陷**：`dirty-data-cases.html` 报 `<path> attribute d: Expected number, "M4.0 NaN"` | **而那是那个页面该有的输出** —— 它的全部意义就是演示"朴素实现会算出 NaN 路径"。**一个故意演示坏输出的页面，会合法地产生错误。** 已按 `source` 收窄到 `network` / `security`（我要补的盲区是"**资源没能加载**"，不是渲染警告），**并再证伪一次**（造一个 import 模块的页面，仍然报红）。⚠️ 顺带修掉一个**极安静的假通过**：加了 `Log.entryAdded` 监听却**没开 `Log.enable`** —— 监听器在，条件永远不成立。 |

...
它正在从"表格与图表的规范"长成**编写前端页面的标准**。怎么长决定它会长成什么，
所以规矩先写在最前面。
...
| 要求 | 为什么 |
|---|---|
| **能回答"它防的是哪个故障"** | 答不上来，说明这是审美偏好而不是规则。**偏好可以写，但要标成偏好** |
| **有判据** —— 能照着判断，不是"要好看" | 判据让两个人在同一个页面上得出同一个结论 |
| **有反例** —— 不合格长什么样 | 只有正例的规则没法自查 |
| **有来源，或者明说它是经验** | 见「本文里的数字分三种」。**③ 允许存在，但不许伪装成 ①** |

**不满足的，宁可先不写。** 这个 skill 现在稀缺的不是覆盖面，是**可信度**：
每条都能追溯到一次真实故障或一份一手规范，读者才敢照做。
**往里塞"我觉得应该这样"，比留白更糟。**

### 现在覆盖什么，不覆盖什么

**已覆盖**（有判据、有反例、有出处或已标为经验）：

- **表格**：规模选型 · 列宽与滚动稳定性 · 密度 · 粘性表头 · 无障碍契约
- **图表**：图形选型 · 轴与基线 · 交互（悬停读数 / 钉住）· 复合图 · 脏数据 · 性能
- **两者共有**：防抖动的稳定性规则 · 主题 token · 深浅两套 · 验证方法

**尚未覆盖**（按缺口排，这就是迭代方向）：

| 维度 | 现状 | 补它的触发条件 |
|---|---|---|
| **页面级布局与栅格** | **§13 起**：目前只有「`min-width: 0`」一条（有实测反例） | 补第二条时同样要有故障来源 |
| **排版阶梯**（字号 / 行高 / 字重） | 只有表格内的三档密度 | 同上 |
| **间距节奏** | 只有局部数值，没有成套系统 | 同上 |
| **表单与控件状态** | 无 | 真做一次带表单的页面 |
| **页面级空态 / 加载态 / 错误态** | 图表侧有，页面级没有 | 同上 |
| **路由与页面切换** | 无 | 出现"切换时闪一下"这类故障时 |

> 补的顺序**跟着真实工作走，不跟着"应该覆盖什么"走**。
> 上表是**待补清单，不是承诺** —— 一次真实故障沉淀下来的东西，比十页"最佳实践"有用。

### 迭代方式

1. **先做完一次真实的页面工作**，不要先想"标准该包含什么"；
2. 把过程中**踩到的坑**和**被迫做的决定**记下来；
3. 满足上面四条门槛的才写进正文，其余进 `references/` 或干脆不写；
4. **改规则时同步改参考实现** —— 正文与范本冲突时 AI 信范本（见 `evidence.md` 的教训）；
5. 每加一条问一次：**这条的出处是什么等级？** 答不上来就标成 ③；
6. **`description` 只在真的覆盖到那个维度之后才扩。** 它是**触发条件** ——
   写宽了会让这个 skill 在它答不上来的任务上被加载，**那比不触发更糟**。
   现在它写的是"表格 / 图表"，因为覆盖到的就是这些；
7. **判据留正文，展开进 `references/`，跨主题的判断进你的知识库。**
   三层的**加载时机不同**：正文**每次任务都付费**；`references/` 由正文指名后加载；
   知识库**不自动加载**，要主动查。判断顺序是 ——
   *"每一轮都会用到吗？"* → 正文 · *"能指名去查吗？"* → `references/` ·
   *"跨 skill，或在回答'当初为什么'？"* → 知识库。
   **别把判据放进知识库**（任务中不会去查，等于没写）；
   **也别把决策记录放进正文**（它占的是每次任务的预算）。
...
> **这一节是新维度。** §0–§12 覆盖表格与图表；这一节起，skill 开始覆盖**页面本身**。
> 它来自一次真实的页面工作，不是"布局应该怎么做"的泛论 —— 所以只有一条规则，
> 而这一条有故障、有判据、有反例、有出处。
...
**原因**（一手规范）：`grid` 与 `flex` 子项的 `min-width` 初始值是 **`auto`**，
在这个上下文里它解析为 **`min-content`** —— "内容再也压不窄的那个宽度"。
所以 `1fr` 实际是 `minmax(min-content, 1fr)`：**你给的份额是期望，不是下限。**
...
**修法**：给**装宽内容的那个子项**加 `min-width: 0`。**加在子项上，不是加在栅格上。**
`flex` 子项同理（或用 `overflow: hidden`，看你要不要保留溢出内容）。

**反例（2026-09-14 实测，900px 视口，`1fr 1fr`）**：

| | 实测 track | 应得 | 行溢出 |
|---|---|---|---|
| 无 `min-width: 0` | **846px** | 438px | **13px** |
| 有 `min-width: 0` | 432px | 438px | 0 |
| 无（`3fr 2fr`） | **846px** | ≈517px | **13px** |
| 有（`3fr 2fr`） | 518px | ≈517px | 0 |

**一份约 120 字符的不可断行文本，就足以把两列栅格撑成单列。**
可复现页面：[`references/min-width-probe.html`](references/min-width-probe.html) ——
打开它、量那四行的宽度，就能看到对照组。

> ⚠️ **这个测试第一次做错了。** 最初的探针用了 60 字符（约 432px），
> 而 `1fr` 分到 438px —— **内容没宽过容器，`min-width: auto` 根本没机会生效**，
> 四行测出来完全一样。**"测了但没测到触发条件"和"没测"一样危险**：
> 它会给你一个"已验证"的错觉。**做反例时先确认反例真的会失败。**

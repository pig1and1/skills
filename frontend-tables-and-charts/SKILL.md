...
> 也不塞"我觉得应该这样"。
> 生长规矩、当前缺口、三层分工的完整判据在**知识库**
> （独立于 skill 的文档目录；本机在会话工作区下，即 `D:\deepseek-harness\knowledge-base\`）：
> `meta/writing-ai-usable-skills.md` 的「教训九」及其中的实例。该文档以本 skill 为唯一案例，
> 标题却是「怎么写一个 AI 真的能用的 skill」—— **在知识库里按标题是找不到它的**。
> **施工时不必读那些** —— 你只需要下面这些判据。
...
**而"没人可问"是常态** —— 这时不许猜，**选一个、把口径写进界面**（做法见 §9 末尾那条）。
...
**色彩走语义 token**（`--table-border` / **`--table-bg`** / `--table-header-bg` /
`--table-row-hover` / `--table-row-selected` / `--table-focus`），深浅两套都要给。

> ⚠️ **`--table-bg` 不是可选的** —— 冻结首列那个 sticky 单元格**自己画背景**，
> 它必须等于表格的静息底色。少了这个 token 而写 `var(--table-bg, Canvas)` 兜底，
> 深色页面下它会回落到系统色、在表格里露出一块浅色。
> （这条由一次独立验证指出：`table.css` 原先两套 token 里都没有它。）
...
**两组实现对跑**的实测（朴素写法 vs 加固写法，9 个病态数据集，朴素版在 **6** 个上失败）。

| 病态 | 朴素写法的后果 | 对策 |
|---|---|---|
| **含 `NaN` / `null` / 字符串** | `Math.min` 拿到非数字 → **整个域变成 `NaN`**，图直接消失 | **先过滤非有限值并计数**，在界面上说明"已忽略 N 条" |
| **时间未排序** | 按原始顺序连线 → 折线来回穿插成乱麻 | **先排序**；同一时间戳应**聚合**而不是留第一条 |
| **时间缺口** | 把空洞**连起来**，图上凭空多出一段不存在的平稳期 —— 这是**撒谎**，不是美观问题 | **先判断它是不是缺口**（见本节后面的「真实的 0 和缺测」），确认是**缺测**才断开：相邻间隔超过中位间隔的 3 倍即视为断开，**分成多段**绘制。⚠️ **这条只对「稀疏上报」成立** —— 对「每日计数」这类，没有记录的那天往往是**真实的 0**，按这条去套会把连续多天 0 单误判成缺测而断开 |
...
**但"没人可问"是常态，不是例外** —— 你在一个隔离的会话里、或者对方就是"你自己看着办"。
这时候**不许猜**，改成**把口径写进界面**：

- 在**图注 / 表头 / 状态区**写明你选了哪一种，以及**为什么数据本身分不出来**；
- 或者**把范围收窄到有覆盖的区间**，让"这里没有数据"在画面上消失；
- 两条都不行（比如必须画满整个区间），就在图上**显式标注"该区间无采样"**。

**判据**：**读者能不能从界面上知道你选了哪个口径？** 能，就没猜；不能，就是猜了。

> ⚠️ **"无人可问"不等于"可以猜"** —— 它只是把"**问**"换成了"**声明**"。
> **默认值也是一个决定，而决定必须让人看得见。**
> 这条同样适用于第 1 步那四个问题：答不上来又没人可问时，选一个、**写出来**、别装作没这回事。

> **来源（D 级 · 实践证据）**：一次独立验证的产物正是被逼到这一步 ——
> 它没人可问，于是**在图注里写明了自己的口径**（"区间内每一天都有采样点：没有任何订单的那天记为 0，
> 不是缺测"）。**它做对了，但那条出路当时只藏在一个表格单元格里。**
...
### 另外五条判断（细节见 [`references/dirty-data.md`](references/dirty-data.md)）

- ★ **"聚合"不等于"求和"** —— 累加型求和、快照型取最后值、**比率要分子分母各自聚合再相除**。
  计数型遇到**修正上报**时，求和会翻倍。
- ★ **分位数要写清算法，否则它会静默失效** —— "用 1%–99% 分位"这句话本身**不够**：
  按**最近秩**（nearest rank）算，`n` 个样本的 99% 分位下标是 `floor((n-1)×0.99)`，
  **样本少时正好落在最大值本人身上** → `value > p99` 永远为假 → **一条异常都标不出来**，
  而界面显示"0 个异常"看起来完全正常。**用线性插值分位，或在界面上写明用的是哪种口径。**
  ⚠️ 这是"判据落在不会因故障而改变的量上"的又一例：**失败的表现是一个合理的 0**。
...
- ★ **"能不能还原"与"上游该不该这么发"是两把尺子，别混用** —— `"18,540"` 是**能无损还原**的
  （去掉分组逗号就是 18540），所以它属于"修复并披露"，不属于"拒绝"。
  **但 `"1.234"` 这种单个分隔符、后面恰好三位的，两个 locale 读出两个值 —— 那才是不可还原的，
  必须问清楚，问不到就判缺位。** 详见 `dirty-data.md`。
...
| **留白够，而且分得清层**（卡片内 20–24px · **区块间 = 卡片内 +4~8** · 页面 32–40px） | 拥挤；**或**每层只差 2–4px —— 那等于没有层级 |
...
| 次级 | **12px** | 620 | 表头 —— **靠字重 + 字色区分表头**（§2） |
| 最次级 | **11px** | 400 | 轴标签、来源行 —— 用最淡的一档颜色 |

**判据**：把页面里所有文字的大小列一遍，**应当只有上面这几档**。
出现第 7 个值、或哪两档只差 0.5px，就是没有阶梯 —— **差 0.5px 不可能是"有意区分"**。

> ⚠️ **字重也只有两个：400 与 620。** 上面表头那一档原本写的是 400 ——
> **那是错的**：它和正文同一个字重，于是 §2 那条"**靠字重 + 字色区分表头**"根本不成立。
> 而六个参考实现里实测用了 **七种**字重（400 / 500 / 550 / 560 / 600 / 620 / 640）——
> **20 处违规**，全部已归位到 400 或 620。（和第 20 轮那次字号散值同一类：**规则立了，
> 但没人拿它量范本**。字号那次量了，字重这次漏了。）

> **两处例外，而且是逐处指定的 —— 不是按类别开的口子**：
> `query-console.html` 的 `.glyph`（**16px 是图标**，不是文字）；
> `stacked-bars-average-line.html` 的 `.tools button`（**按钮文字 12.5px**）。
> **阶梯管的是内容层级，不是所有 `font-size`。** 这两处是 2026-09-14 把参考实现
> 逐个对齐到阶梯上时**故意留下**的，其余 13 处散值已全部归位。
>
> ⚠️ **不要把"控件外观"当成一个类别来豁免。** 上面第二处**只豁免那一个按钮**，
> 不是"凡是控件上的字号都可以不上阶梯"。控件外观确实是本 skill 还没覆盖的维度
> （见知识库 `meta/writing-ai-usable-skills.md` 的缺口表），但 **"没覆盖"不等于"随便写"** —— 没有依据时，
> 用阶梯上已有的档，比新造一个 12.5px 更安全。
> **这一条由第 5 次独立验证指出：开给"一个类别"的例外，会比规则本身管得还宽。**
...
每节都给了判据，这里**只列最容易漏掉的**。**四个必做**不能跳：

1. **先让它跑起来** —— 打开页面，看控制台有没有**未捕获异常**。
   **静态检查全绿不代表页面是活的**：`dashboard.html` 曾在一个变量名上抛异常、
   三个图表一个都没画，而自检清单当时一条都没拦住（见 `references/reference-smoke.mjs`）。
   **这一条要排在其他所有检查之前。**
2. **真机滚动一遍**（§1）—— 不是看一眼，是真的滚。列宽 / 行高 / 滚动条位置都不许动。
3. **喂一批脏数据**（§9）—— `NaN`、乱序、全同值、空集、缺口各来一个。不抛异常，并如实报告丢了几个。
4. **按实际窗口宽度看**（§11）—— 至少验常规窗口与缩窄后两档。最大化窗口下能看不等于能用。

其余八条：

- [ ] 你写的每条检查，**先确认它在当前条件下真的能失败**吗？
      例：一条"横向滚动后首列不动"的断言 —— 如果表格总宽本来就没超出容器，**它恒真**，
      和"没检查"完全一样。做法是做一个**变异**：把被保护的东西改坏，看它会不会红。
      **"测了但没测到触发条件"和"没测"一样危险**（见 `evidence.md` 的"踩过的坑"）。
...
- [ ] 用下面几条命令扫过一遍吗？**一条规则如果变不成一条命令，就等于检查不了。**
      ...
      # ⚠️ 这四条**都只列候选，不判合格** —— 每一条都要走第二步，见下方说明。
      # ⚠️ 也是 POSIX 写法。**Windows / PowerShell 下没有 `grep`**，等价替换是
      #    `Select-String -Path x.html -Pattern '…'`；`grep -v` 对应
      #    `| Where-Object { $_.Line -notmatch '…' }`。
      #    **照抄一条在当前机器上跑不起来的命令，等于没检查。**
      ...
      **第一步是这个 grep，第二步才是判据** —— 这四条命令都会给出"看着像违规、其实不是"的行：

      | 命中 | 第二步怎么看 |
      |---|---|
      | 字面 `<text` | 看**它所在的那个 SVG** 有没有 `preserveAspectRatio="none"`（或有 `viewBox` 且被 CSS 拉伸）。**用 `meet` 的、或干脆不写 `viewBox` 的，`<text>` 是安全的。** 注释里提到这个词也会命中 |
      | `table-layout` 非 `fixed` | ⚠️ **这条最容易给假绿灯。** `grep -v fixed` 会把**同一行里出现 `fixed` 的任何行整行滤掉** —— 包括 `table-layout: fixed; /* … */` 这种带注释的写法，也包括**真正的违规行里恰好提到 fixed**。**看到"无输出"先别当干净**：单跑一遍 `grep -n 'table-layout' x.html` 对照。（第 5 次独立验证指出的：判据落在了一个会因无关内容而改变的量上。）另外要排除**故意的对照组** —— `references/verify.html` 里就有一个按钮专门切到 `auto`，用来演示"关掉规则 1 会怎样" |
      | 字面色值 | 自包含页面**必然**在 `:root` 里写字面色值。判据是"只出现在 token 定义块里"，grep 替不了这一步 |
      | `display = ''` | 看**那个元素的 CSS 有没有 `display: none`**。有 → 是真的回落到 `none`（bug）；没有 → `''` 只是清掉上一个内联值，是**正确**的 |

      > **用 `hidden` 属性或 `classList` 切换显示，是更好的做法** —— 那条 grep 不认它，
      > 而**那正是它该有的样子**：这个 grep 找的是"一种常见错法"，不是"所有正确写法"。
      > **没被它命中 ≠ 审核过了。**（第 6 次独立验证的构建者用了 `hidden` 属性，
      > 并指出这条清单"既不报警也不认可"它。）

      > **这四条命令我自己在参考实现上跑过一遍**（2026-09-14）：`display = ''` 报了 4 处，
      > **3 处是真 bug**（两个 tooltip + 一条十字准线，CSS 里都写着 `display:none`），
      > 1 处是合法的（`#brush` 没有 `display:none`）；`<text` 报了 4 处，
      > **2 处是违规**（直方图的端点标签塞在 `none` 的 SVG 里）、2 处安全。
      > **真缺陷全部已修。**
      >
      > `verify.html` 还会报 2 处 `table-layout` 非 `fixed` —— 那是**故意的对照组**，不是缺陷。
      > **四个页面跑下来，四条命令一共报了 12 处，其中 5 处是真问题、7 处是合法的。**
      > 命令没骗人，但**只跑第一步的话，一半以上是噪音** —— 而一个总在喊狼来了的清单，
      > 会被整份无视。**所以第二步不是可选项。**
- [ ] 深浅两套主题都验过吗？
...
> ⚠️ **下面这一小段是「当时那份验证报告的原话」，不是现行声称** ——
> 它引用的"§9 说…"**已经被紧接着的补记更正**。留着它，是因为它记录了缺口是怎么被发现的。
>
> （**这个标注是有理由的**：第 6 次独立验证的构建者把下面这段引文读成了现行规则，
> 在自己的报告里写成"§13 声称 §9 说了某事"。**引用旧结论时，必须让人一眼看出它是旧的。**）
>
> > 做导入页的 AI 发现：**§13 说"会被撑破"，§9 说"截断并挂 title"，
> > 叠加时两节没有统一说法**，只能自己拍板。**分别都对，合起来没讲 —— 这本身就是缺口。**
...
**`aria-busy` 什么时候清？** —— **和"写下最终状态"是同一个地方、同一个时刻。**
- 开始重绘 / 重新取数 → `aria-busy="true"`；
- **拿到结果、把最终文案写进去的那一刻 → 清回 `false`**（或移除该属性）。
- ⚠️ **成败都要清** —— 失败分支不清，辅助技术会**一直以为这个区域还在忙**，
  于是**后续更新不再播报**：一次失败把这块区域永久静音了。

**反例**：只设不清 → 界面看起来早就好了，而读屏软件仍认为它在加载中。
（第 6 次独立验证指出：技能只讲了"设"，没讲"清"，它只能自己定。）

> ⚠️ **别把"清 `aria-busy`"和"清状态文字"写在一起。**
> 上面那条陷阱说的是文字**不许**在收尾逻辑里被清空；而 `aria-busy` **必须**被清。
> 两者方向相反，所以判定位置也不同 —— **写最终文案的地方**同时清 `aria-busy`，
> 而**不要**另找一个 `finally` 去统一清理。
...
- `references/run-all-checks.mjs` —— **一次跑完本目录所有检查**，给一张总表。
  它按**三种**结果归类：通过 / 失败 / **未校验**（例如缺 `tsc` 时的 LineChart 检查）——
  **把"未校验"混进"通过"就是假通过**。需要外部静态服务的那个检查，由它自己起服务。
- `references/reference-smoke.mjs` —— **冒烟测试：把 `references/` 下每个 HTML 真跑一遍**，
  只断言一件事：**加载后没有未捕获异常**。它是所有检查里最基础的一条，也是**唯一能发现
  "页面根本跑不起来"的那条** —— `dashboard.html` 曾经加载即抛异常、三个图表一个都没画，
  而静态检查（自包含、引用解析、字号、四条 grep）**全部通过**。零依赖，CDP + `file://`。
- `references/linechart-check.mjs` —— **把 `LineChart.ts` 真编译、真跑一遍**。
  判据是 canvas 三个区域**都有墨**：左槽（Y 标签）、底槽（X 标签）、绘图区（折线）。
  ⚠️ 加它的原因：这个骨架**全库没有消费者** —— 没人 import 它、也没人打开它，
  所以 `tsc --noEmit` 通过**根本不能说明它还能画**。它是全库唯一需要 `tsc` 的检查，
  **找不到编译器时报"未校验"并 exit 0，不假装通过**。
- `references/composite-chart-check.mjs` —— **逐条验证上面那份复合图示例的 6 条声称**：
  三段堆叠 · 与折线**共用从 0 起的轴** · 整柱透明命中区 · **只有顶段有曲线** ·
  **选中在鼠标移开后保持**（区别于悬停）· 图例全隐藏不留空图。
  用 CDP 派发**真鼠标事件** —— "悬停 vs 选中并保持"这一条只有真事件测得出来。零依赖。

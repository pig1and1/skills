...
> ⚠️ **上表的命中数是量级快照，会随索引漂移。**要看的是**量级与相对关系**
> （33 与 80 的差别有意义，33 变成 41 没有），**不是具体数字**。别把数字当判据。
...
一次搜索回来 20 条，**不要按顺序读**。GitHub 自带几个顺手可用的信号：

| 信号 | 它回答什么 | 查询写法 |
|---|---|---|
| **仓库 stars** | 这份材料被多少人用过 | 看 `repo:` 命中的仓库 |
| **👍 reactions** | **有多少人也遇到同一个问题** —— 比 star 更贴近"这坑真实存在" | `reactions:>10` |
| **评论数** | 讨论是否充分 | `comments:>10` |
| **已关闭 + 有 linked PR** | **真被解决了**，还是被搁置 | `is:closed`，再看有没有 PR 引用它 |
| **是否官方仓库** | 规范的权威方 > 任何第三方转述 | `repo:owner/name` |

**两类结果要两套判据** —— 这条是硬经验：

- **规范 / 实现参考** → **用 stars 衡量可信度**。例如 Ant Design 99.5k★、Carbon 9.5k★；
  而一份**没有 stars 可查**的第三方汇总文档，**过不了这道门槛**。
- **故障证据**（某人的 issue / PR）→ **不设 stars 门槛**。一条 issue 只要现象可复现就是硬证据，
  哪怕来自 0★ 仓库。**但措辞要跟着改**：不是"很多人踩过"，而是"某个几乎无人使用的项目踩过"
  —— 于是必须多问一句：**这是平台/语言原理，还是那个项目的特殊性？**

> **实测反例**：`"windowsHide" "SW_HIDE"` 的结果里，最有用的线索来自小仓库，但它讲的是
> Windows `CreateProcess` 的语义 —— **原理问题，普适**。同样来自小仓库的
> "Safari 快滚时粘性表头抖 1–2px" 却是 **WebKit 怪癖，不普适**。
> **stars 区分不出这两种**，只能靠"这是原理还是特例"来判断。

**stars 不是万能的**：它衡量"被广泛使用"，不衡量"这条断言是否为真"。
Ant Design 有 99.5k★ 说明它权威，但"它的默认行高是多少"仍要打开它的源码才能确认
（结论是它有 `large` / `middle` / `small` 三档）。**流行度替代不了那一行代码。**
